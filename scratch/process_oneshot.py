import json
import os
import shutil

# Dữ liệu JSON đã được giải và highlight theo Ma trận Lục hợp (ĐÃ SỬA LỖI 4 CÂU)
data = [
    {
        "weight": "normal",
        "tags": ["Tài chính doanh nghiệp"],
        "question": "Mục đích tài chính cuối cùng của các doanh nghiệp là <span class=\"keyword\">tối đa hóa</span>",
        "options": [
            "A. <span class=\"answer-keyword\">lợi nhuận</span>.",
            "B. tài sản.",
            "C. các chỉ số tài chính.",
            "D. doanh thu."
        ],
        "answer": "<div class=\"answer-title\">✅ Đáp án:</div> A. lợi nhuận.\n<div class=\"note\">Theo giáo trình, mục tiêu tài chính cuối cùng của doanh nghiệp được định hướng là tối đa hóa lợi nhuận.</div>"
    },
    {
        "weight": "normal",
        "tags": ["Bản chất tài chính"],
        "question": "Sự tồn tại của nền sản xuất hàng hóa – tiền tệ và Nhà nước được coi là những <span class=\"keyword\">tiền đề phát sinh</span> và phát triển của",
        "options": [
            "A. tổ chức tài chính trung gian.",
            "B. ngân hàng.",
            "C. <span class=\"answer-keyword\">tài chính</span>.",
            "D. tiền tệ."
        ],
        "answer": "<div class=\"answer-title\">✅ Đáp án:</div> C. tài chính.\n<div class=\"note\">Sự ra đời của nền sản xuất hàng hóa - tiền tệ và sự xuất hiện của Nhà nước là 2 tiền đề khách quan quyết định sự ra đời, tồn tại và phát triển của tài chính.</div>"
    },
    {
        "weight": "normal",
        "tags": ["Vai trò tài chính"],
        "question": "Ý nào sau đây <span class=\"keyword\">KHÔNG phải</span> vai trò của tài chính ở tầm <span class=\"keyword\">vi mô</span>?",
        "options": [
            "A. <span class=\"answer-keyword\">Tăng cường tiềm lực</span> tài chính của đất nước.",
            "B. Điều tiết kinh tế.",
            "C. Tích tụ và tập trung vốn, cung ứng vốn cho các nhu cầu công nghiệp hóa, hiện đại hóa.",
            "D. Xác lập và tăng cường các quan hệ kinh tế - xã hội."
        ],
        "answer": "<div class=\"answer-title\">✅ Đáp án:</div> A. Tăng cường tiềm lực tài chính của đất nước.\n<div class=\"note\">\"Tăng cường tiềm lực tài chính của đất nước\" là vai trò mang tính vĩ mô (Tài chính nhà nước/Tài chính công).</div>"
    },
    {
        "weight": "normal",
        "tags": ["Nguồn tài chính"],
        "question": "Những <span class=\"keyword\">tiềm năng tài chính</span> mà các chủ thể trong xã hội có thể khai thác, sử dụng nhằm thực hiện các mục đích của mình là",
        "options": [
            "A. các quỹ tiền tệ.",
            "B. các khâu tài chính.",
            "C. các chủ thể phân phối.",
            "D. <span class=\"answer-keyword\">nguồn tài chính</span>."
        ],
        "answer": "<div class=\"answer-title\">✅ Đáp án:</div> D. nguồn tài chính.\n<div class=\"note\">Nguồn tài chính là tiềm năng tài chính (các nguồn tiền) mà các chủ thể trong xã hội có thể huy động, khai thác và sử dụng để lập nên các quỹ tiền tệ phục vụ mục đích của mình.</div>"
    },
    {
        "weight": "normal",
        "tags": ["Bản chất tài chính"],
        "question": "<span class=\"keyword\">Tài chính là gì</span>?",
        "options": [
            "A. Tài chính là các chứng khoán.",
            "B. Tài chính là tiền và các loại ngoại tệ.",
            "C. Tài chính là các luồng tiền.",
            "D. Tài chính là <span class=\"answer-keyword\">sự vận động của vốn</span> tiền tệ."
        ],
        "answer": "<div class=\"answer-title\">✅ Đáp án:</div> D. Tài chính là sự vận động của vốn tiền tệ.\n<div class=\"note\">Bản chất của tài chính là các quan hệ kinh tế dưới hình thái giá trị phát sinh trong quá trình hình thành và sử dụng các quỹ tiền tệ (sự vận động độc lập tương đối của vốn tiền tệ).</div>"
    },
    {
        "weight": "high",
        "tags": ["Ngân sách Nhà nước"],
        "question": "Khoản thu nào dưới đây <span class=\"keyword\">chiếm tỷ trọng lớn nhất</span> trong tổng thu ngân sách nhà nước ở Việt Nam?",
        "options": [
            "A. phí.",
            "B. <span class=\"answer-keyword\">thuế</span>.",
            "C. sở hữu tài sản.",
            "D. lệ phí."
        ],
        "answer": "<div class=\"answer-title\">✅ Đáp án:</div> B. thuế.\n<div class=\"note\">Thuế là nguồn thu chủ yếu, mang tính bắt buộc và chiếm tỷ trọng lớn nhất trong tổng thu Ngân sách Nhà nước.</div>"
    },
    {
        "weight": "normal",
        "tags": ["Ngân sách Nhà nước"],
        "question": "Đâu là <span class=\"keyword\">khâu cuối cùng</span> trong chu trình quản lý ngân sách nhà nước?",
        "options": [
            "A. <span class=\"answer-keyword\">Quyết toán</span> ngân sách",
            "B. Chấp hành dự toán chi",
            "C. Chấp hành dự toán thu",
            "D. Hình thành ngân sách"
        ],
        "answer": "<div class=\"answer-title\">✅ Đáp án:</div> A. Quyết toán ngân sách\n<div class=\"note\">Chu trình quản lý NSNN gồm 3 khâu: Lập dự toán ➔ Chấp hành dự toán ➔ Quyết toán ngân sách (khâu cuối cùng).</div>"
    },
    {
        "weight": "normal",
        "tags": ["Ngân sách Nhà nước"],
        "question": "<span class=\"keyword\">Phí</span> thuộc ngân sách nhà nước thu về",
        "options": [
            "A. đủ để bù đắp chi phí đã bỏ ra.",
            "B. <span class=\"answer-keyword\">không đủ bù đắp</span> chi phí đã bỏ ra.",
            "C. không tính tới chi phí đã bỏ ra.",
            "D. vượt quá chi phí đã bỏ ra."
        ],
        "answer": "<div class=\"answer-title\">✅ Đáp án:</div> B. không đủ bù đắp chi phí đã bỏ ra.\n<div class=\"note\">Trong tài chính công, phí do nhà nước thu thường mang tính chất phục vụ công cộng nên khoản thu về thường không đủ bù đắp toàn bộ chi phí đã bỏ ra.</div>"
    },
    {
        "weight": "normal",
        "tags": ["Ngân sách Nhà nước"],
        "question": "Những <span class=\"keyword\">khoản mục thu thường xuyên</span> trong cân đối ngân sách nhà nước bao gồm:",
        "options": [
            "A. <span class=\"answer-keyword\">thuế</span>, <span class=\"answer-keyword\">sở hữu tài sản</span>, phí và lệ phí.",
            "B. thuế, sở hữu tài sản, phí và lệ phí, phát hành trái phiếu chính phủ.",
            "C. thuế, sở hữu tài sản, phí và lệ phí, lợi tức cổ phần của Nhà nước.",
            "D. thuế, phí và lệ phí, từ các khoản viện trợ có hoàn lại."
        ],
        "answer": "<div class=\"answer-title\">✅ Đáp án:</div> A. thuế, sở hữu tài sản, phí và lệ phí.\n<div class=\"note\">Các khoản thu thường xuyên chính của Ngân sách Nhà nước chủ yếu là Thuế, Phí, Lệ phí và Thu từ sở hữu tài sản.</div>"
    },
    {
        "weight": "normal",
        "tags": ["Ngân sách Nhà nước"],
        "question": "Trong các khoản chi sau, khoản chi nào là thuộc <span class=\"keyword\">chi thường xuyên</span>?",
        "options": [
            "A. Chi khoa học, công nghệ và môi trường.",
            "B. Chi giải quyết việc làm.",
            "C. Chi <span class=\"answer-keyword\">dân số kế hoạch hóa</span> gia đình.",
            "D. Chi trợ cấp ngân sách cho phường, xã."
        ],
        "answer": "<div class=\"answer-title\">✅ Đáp án:</div> C. Chi dân số kế hoạch hóa gia đình.\n<div class=\"note\">Chi dân số kế hoạch hóa gia đình thuộc nhóm chi sự nghiệp y tế, xã hội - một bộ phận của chi thường xuyên nhằm duy trì hoạt động xã hội.</div>"
    }
]

subject_dir = '/Users/thien-ban/Library/CloudStorage/OneDrive-Personal/03_WORK/11. LearnIZ/_sources/TVU/Quan_tri_kinh_doanh_DH/15. TÀI CHÍNH TIỀN TỆ'
staging_file = os.path.join(subject_dir, 'staging', 'temp_qs.json')
main_file = os.path.join(subject_dir, 'qs.json')

# Copy to staging
with open(staging_file, 'w', encoding='utf-8') as f:
    json.dump(data, f, ensure_ascii=False, indent=2)

# Verify parsing (in this case it's dict directly, but simulate parse check)
if isinstance(data, list):
    # Overwrite main
    shutil.copy2(staging_file, main_file)
    print("DONE: Processed and fixed 10 questions and wrote to qs.json.")
