...
- **Bước 7:** **User** quăng toàn bộ kiến thức, tài liệu giáo trình vào thư mục môn học rồi chốt lệnh (VD: "Chốt"). **ĐẶC BIỆT LƯU Ý:** User phải ƯU TIÊN nạp file **KẾ HOẠCH HỌC TẬP (Syllabus/Đề cương chi tiết)** nếu có. File này là "Thánh Chỉ" cao nhất để định hình trọng tâm. NẾU User hô *"Bỏ qua"* hoặc *"Nhảy qua đề thi"*, Agent lập tức BỎ QUA Bước 8 & 9 và nhảy thẳng sang Bước 10.
...
- **Bước 8.5 (Checkpoint Duyệt Bản Nháp — CỬA CHẶN BẮT BUỘC):** Sau khi Dây chuyền Bước 8 hoàn tất rà soát ngầm, Agent Trưởng **PHẢI DỪNG LẠI** và xuất **Bản Nháp Trực Quan Hóa Tổng Thể** theo template chuẩn bên dưới. Trình User xem và chờ lệnh **"Duyệt"** rồi mới được tiến hành Bước 9.
  ...
  - Lệnh cấm cực ngặt: **Tuyệt đối CẤM sử dụng component `mindmap` bên trong khối `details` (Mindmap Inception).** Mindmap CHỈ ĐƯỢC PHÉP dùng ở `overview`.
  - Chống Ảo giác Component: **TUYỆT ĐỐI CẤM tự bịa ra tên Component (như `accordion`, `comparison_table`...) theo thói quen.** BẮT BUỘC phải tra cứu và bê chính xác 100% tên Component được định nghĩa trong `knowledge-app/src/types/schema.ts` (ví dụ: `features`, `matrix-table`). Phải soi kỹ cấu trúc (vd: `features` xài `description`, không xài `content`) trước khi ráp dữ liệu.
  - Dọn rác dữ liệu: **CẤM để sót lại các thẻ đánh dấu của LLM (kể cả ngoặc vuông `([Logic])` lẫn ngoặc tròn `(Logic)`, `(Quy trình)`, `(Đa chiều)`) trong bất kỳ trường `label`, `description` hay nội dung nào của dữ liệu.** Tuyệt đối KHÔNG ĐƯỢC lười biếng dùng các thẻ phân loại này làm nội dung cho cột `description` (nếu Node không có mô tả thì để chuỗi rỗng `""`).
...
- **Bước 10.5 (Checkpoint Ma Trận Đề Thi — THE VETERAN PROFESSOR BLUEPRINT):** BẮT BUỘC DỪNG LẠI CHỜ DUYỆT trước khi xử lý câu hỏi.
  ...
  - File `blueprint.json` phải quy định rạch ròi 2 thứ: (1) **Định chuẩn 80/20 (Tuyệt đối CẤM Agent dùng kiến thức nền tự phán đoán trọng tâm)**: Cơ chế xác định mác High tuân theo 2 mỏ neo: [Ưu tiên 1] Dựa vào chuẩn đầu ra, số tiết, trọng điểm trong file **Kế Hoạch Học Tập**. [Ưu tiên 2] Nếu không có kế hoạch học tập, lấy danh sách các Node trong **`kb.json`** làm mỏ neo tuyệt đối (Có Node = High, râu ria ngoài Node = Normal); (2) **Ma trận Đề thi** (Quy định tỷ lệ phần trăm phân bổ câu hỏi theo từng Tag kiến thức chuyên đề).
...
- **Bước 11:** **Agent Trưởng** kích hoạt Dây chuyền Xử lý Đề 1 Chạm:
  ...
  - **Quy tắc Liên kết Kép & Gán thẻ High (Weight):** 
    - 1. **Đồng bộ Tag:** Trước khi dán Tag phân loại cho `qs.json`, Agent BẮT BUỘC phải dùng `view_file` đọc lại `kb.json` để soi Tên Chương/Chủ Đề đã chốt. Tag chuyên đề trong `qs.json` phải KHỚP 100% (cả dấu và viết hoa/thường) với tag trong `kb.json`. Cấm tự chế tên tag mới gây đứt gãy bộ lọc UI.
    - 2. **Xác định Trọng tâm:** Khi đang phân tích câu hỏi, Agent BẮT BUỘC đối chiếu nội dung câu đó với mỏ neo `blueprint.json`. Nếu thoả mãn định chuẩn 80/20 (Kiến thức gốc rễ, Bẫy kinh điển, Tính bao quát cao) ➔ Gán thuộc tính `"weight": "high"`. Nếu không thoả mãn ➔ Tuyệt đối không gán thuộc tính `weight` (để trống). Không còn xài thẻ `normal` hay tag thủ công `80/20`.
