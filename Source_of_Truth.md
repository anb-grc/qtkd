...
- **Bước 11:** **Agent Trưởng** kích hoạt Dây chuyền Xử lý Đề 1 Chạm (Nhận diện Tàng hình):
  ...
  - **Quy tắc Liên kết Kép & Gán thẻ High (Weight):** 
    ...
    - 2. **Xác định Trọng tâm:** Khi đang phân tích câu hỏi, Agent BẮT BUỘC đối chiếu với mỏ neo Vi Mô trong `blueprint.json`. Một câu hỏi chỉ được gán thẻ `"weight": "high"` khi nó đâm trúng 20% cốt lõi VÀ thuộc 1 trong 3 dạng (Tam giác vàng): (1) Bản chất Xương sống (Core Concept); (2) Logic Vận hành / Công thức (Mechanism); (3) Điểm Phân Biệt / Ranh Giới (Differentiation). Nếu KHÔNG thoả mãn ➔ TUYỆT ĐỐI BỎ TRỐNG (không gán thuộc tính `weight`). Hệ thống mặc định hiểu đó là câu thường. Cấm tự ý tạo ra giá trị `"normal"` đắp vào file. Bổ sung: Nếu quét xong rổ câu hỏi mà phát hiện một Trụ cột bị "trắng" câu High, Agent phải đưa cảnh báo Đỏ vào Báo Cáo Nghiệm Thu, tuyệt đối cấm đôn câu rác lên làm High cho đủ KPI.
