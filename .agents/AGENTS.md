...
13. **BẢO VỆ GIAO DIỆN IN HOA "LESS IS MORE":** Mọi chuỗi VIẾT IN HOA (`text-transform: uppercase`) phải rọi về một tone màu chuẩn là Xanh Sáng (`var(--color-accent-secondary)`), trừ thẻ cảnh báo bẫy lật kèo dùng Đỏ (`--color-danger`). Văn bản chính Trắng (`#ffffff`), giải thích dài Xám mờ (`rgba(255,255,255,0.7)`). Cấm chế cháo màu rải thảm lên tiêu đề hay chèn icon/emoji rác rưởi gây xao nhãng thị lực.
